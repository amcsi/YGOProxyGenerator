...
describe('preferDb storage', () => {
  it('defaults to false when missing or corrupt', () => {
    const mem = {
      getItem: () => null
    };
    assert.equal(readPreferDb(mem), false);
    assert.equal(
  ...
  it('reads true only for string true', () => {
    assert.equal(
...
describe('parseYgoProDeckCardInfo', () => {
  it('returns null for error JSON that still has HTTP-style success body', () => {
    assert.equal(
      parseYgoProDeckCardInfo(
  ...
  it('returns parsed payload when data[0] exists', () => {
    const payload = parseYgoProDeckCardInfo(
      '{"data":[{"id":35844557,"card_images":[{"image_url":"https://images.ygoprodeck.com/images/cards/35844557.jpg"}]}]}'
    );
    assert.equal(payload.data[0].id, 35844557);
  });
});
